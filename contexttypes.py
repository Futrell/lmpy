...
class DependencyContext(object):
    defaultParser = DependencyParser
...
class PositionalContext(object):
    defaultParser = PositionalParser
...
class ContextParser(object):
    ...
    def __init__(self, lemmatize=None, boundaries=False, 
                 **kwargs):
        try:
...
class DepsParser(ContextParser):
    contextType = DependencyContext

    def __init__(self, lemmatize=None, **kwargs):
        self.tokenize = lambda x : x.split(" ")
        self.lemmatize = lemmatize
        if 'preLemmatized' in kwargs:
